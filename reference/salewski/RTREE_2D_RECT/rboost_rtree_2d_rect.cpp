...
static VALUE rtree_nearest(VALUE self, VALUE x, VALUE y)
{
	RTC *rt;
	Data_Get_Struct(self, RTC, rt);
	std::vector<RTE> result;
	rt->query(bgi::nearest(point(NUM2DBL(x), NUM2DBL(y)), 1), std::back_inserter(result));
	return result[0].second;
}

static VALUE rtree_nearest_k(VALUE self, VALUE x, VALUE y, VALUE k)
{
	long kk;
	if ((kk = NUM2LONG(k)) <= 0)
...
static VALUE rtree_nearest_k_rect(VALUE self, VALUE x, VALUE y, VALUE k)
{
	long kk;
	if ((kk = NUM2LONG(k)) <= 0)
		rb_raise(rb_eArgError, "FIXNUM > 0 expected!");
...
static VALUE rtree_nearest_rect(VALUE self, VALUE x, VALUE y)
{
	RTC *rt;
	Data_Get_Struct(self, RTC, rt);
	std::vector<RTE> result;
	rt->query(bgi::nearest(point(NUM2DBL(x), NUM2DBL(y)), 1), std::back_inserter(result));
	return rb_ary_new3(5, result[0].second, 
		rb_float_new(result[0].first.min_corner().get<0>()),
		rb_float_new(result[0].first.min_corner().get<1>()),
		rb_float_new(result[0].first.max_corner().get<0>()),
		rb_float_new(result[0].first.max_corner().get<1>()));
}

static VALUE rtree_nearest_k_each(VALUE self, VALUE x, VALUE y, VALUE k)
{
	long kk;
	if ((kk = NUM2LONG(k)) <= 0)
		rb_raise(rb_eArgError, "FIXNUM > 0 expected!");
...
static VALUE rtree_nearest_k_rect_each(VALUE self, VALUE x, VALUE y, VALUE k)
{
	long kk;
	if ((kk = NUM2LONG(k)) <= 0)
		rb_raise(rb_eArgError, "FIXNUM > 0 expected!");
...
extern "C" void Init_rboost_rtree_2d_rect() {
	...
	rb_define_method(cRT2dR, "nearest_k", (ruby_method*) &rtree_nearest_k, 3);
	rb_define_method(cRT2dR, "nearest", (ruby_method*) &rtree_nearest, 2);
	rb_define_method(cRT2dR, "nearest_k_each", (ruby_method*) &rtree_nearest_k_each, 3);
	rb_define_method(cRT2dR, "nearest_rect", (ruby_method*) &rtree_nearest_rect, 2);
	rb_define_method(cRT2dR, "nearest_k_rect", (ruby_method*) &rtree_nearest_k_rect, 3);
	rb_define_method(cRT2dR, "nearest_k_rect_each", (ruby_method*) &rtree_nearest_k_rect_each, 3);
}
