...
//
// Initially I wrote a Queue with these minor limitations:
...
//
// This one is a modified Queue, it is supported by ruby Hashes.
// We put each object additional in a hash -- objects equal for hashes are considered
// equal for the priority queue too. So you can use string constants or arrays like [obj, true]
// (which are considered the same for hash keys while it are indeed different objects) and insert
// it in the queue -- it its only inserted once, inserting again will update only the key.
// I think this is as Brian Schroeder did it -- of course some overhead in storage and acces time.
//
// - Key datatype for priority is float (you can not directly use Strings as keys...)
//
// c Stefan Salewsk, mail@ssalewski.de
// License GPL
// Version 0.2 03-SEP-2013
...
void fq_push(XPQ *pq, VALUE d, VALUE key)
{
...
extern "C" VALUE fqueue_push(VALUE self, VALUE node, VALUE key)
{
	XPQ *pq;
	Data_Get_Struct(self, XPQ, pq);
	if (NIL_P(rb_hash_aref(pq->hash, node)))
		fq_push(pq, node, key);
	else
		rb_raise(rb_eRuntimeError, "Fibonacci_Queue.push(): node is already in this queue!");
	return Qnil;
}
...
extern "C" VALUE fqueue_delete(VALUE self, VALUE node)
{
	...
	else
	{
		...
		pq->erase(h->handle);
		rb_hash_delete(pq->hash, node);
	}
	return Qtrue;
}

void fq_update(XPQ *pq, VALUE hd, VALUE key)
{
	heap_data* h;
	Data_Get_Struct(hd, heap_data, h);
...
void fq_increase(XPQ *pq, VALUE hd, VALUE key)
{
	heap_data* h;
	Data_Get_Struct(hd, heap_data, h);
	(*h->handle).payload = convert_key(key) * pq->key_sign;
...
void fq_decrease(XPQ *pq, VALUE hd, VALUE key)
{
	heap_data* h;
	Data_Get_Struct(hd, heap_data, h);
	(*h->handle).payload = convert_key(key) * pq->key_sign;
...
extern "C" VALUE fqueue_update(VALUE self, VALUE node, VALUE key)
{
	XPQ *pq;
	Data_Get_Struct(self, XPQ, pq);
	VALUE hd = rb_hash_aref(pq->hash, node);
	if (NIL_P(hd))
		return Qfalse;
	else
		fq_update(pq, hd, key);
...
extern "C" VALUE fqueue_clear(VALUE self)
{
	...
	Data_Get_Struct(self, XPQ, pq);
	pq->clear();
	//rb_hash_clear(pq->hash); not available for ruby 1.9
	pq->hash = rb_hash_new(); // so use a new empty hash
	return Qnil;
}
...
extern "C" VALUE fqueue_delete_top(VALUE self)
{
	...
	const heap_data& hd = pq->top();
	rb_hash_delete(pq->hash, hd.data);
	pq->pop();
	return Qtrue;
}
